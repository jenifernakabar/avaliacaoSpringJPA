...
public class ProdutoRest {
	...
	public ResponseEntity atualizarProduto(@PathVariable String idRequest, @RequestBody Produto produto) {
		try {
			Optional<Produto> produtoDB = produtoRepository.findById(Long.valueOf(idRequest));
			if (produtoDB.isPresent()) {
				...
				return new ResponseEntity<>(new ApiMessage("Produto " + produtoDB.get().getNome() + " Atualizado", 204),HttpStatus.ACCEPTED);
			} else {
				return new ResponseEntity<>(new ApiMessage("Produto Não Encontrado", 400), HttpStatus.BAD_REQUEST);
			}
	...
	public ResponseEntity atualizarEstoque(@PathVariable String id,
			@PathVariable String quantidadeEstoque) {
		try {
